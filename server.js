...
app.get("/s/:token", async (req, res) => {
    try {
        const t = (req.params.token || "").trim();
        if (!t) return res.status(400).send("Token inválido");
       ...
       limit 1`,
        ...
        if (r.rowCount === 0) {
            return res.status(404).send("Link caducado o no válido");
        }
        ...
        if (!frontend) {
            return res
                .status(500)
                .send("FRONTEND_URL no configurado en el servidor");
        }

        return res.redirect(`${frontend}/?share=${encodeURIComponent(t)}`);
    } catch (e) {
